...
void Queries(SegmentTree segment_tree, int n){
  cout << segment_tree.RangeQuery(0, 0, n-1, 0, 1) << endl;
  cout << segment_tree.RangeQuery(0, 0, n-1, 0, 2) << endl;
  cout << segment_tree.RangeQuery(0, 0, n-1, 0, 3) << endl;
  cout << segment_tree.RangeQuery(0, 0, n-1, 1, 2) << endl;
  cout << segment_tree.RangeQuery(0, 0, n-1, 1, 3) << endl;
  cout << segment_tree.RangeQuery(0, 0, n-1, 2, 3) << endl;
}
int main(){
  int A[] = {1,3,4,2};
  ...
  //Queries(segment_tree, n);
  segment_tree.RangeUpdate(0, 0, n-1, 1, 2, 1);
  Queries(segment_tree, n);
  return 0;
}
